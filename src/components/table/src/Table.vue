...
import type { TableCellChange, TableColumn, TableColumnSetting, TableDirtySubmitPayload, TableFixed, TableProps, TableQueryChangePayload, TableRowDblclickPayload, TableRowKey, TableSize, TableStoredState } from './types'
...
const columnPanelMaxHeight = computed(() => {
  if (!tableRootHeight.value) {
    return '420px'
  }

  return `${Math.max(120, tableRootHeight.value - 16)}px`
})
...
const defaultEmptyImage =
  "data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='240' height='120' viewBox='0 0 240 120'><rect width='240' height='120' rx='14' fill='%23f7f8fa'/><g transform='translate(68 38)'><rect width='34' height='34' rx='8' fill='%23b8c7cf'/><path d='M10 10l14 14M24 10L10 24' stroke='%23ffffff' stroke-width='3.2' stroke-linecap='round'/><text x='42' y='23' font-size='18' fill='%23b1b9c3' font-family='Arial, sans-serif'>x.ui</text></g></svg>"
...
const headerDividerStyle = computed(() => (props.showHeaderVerticalDivider ? '1px solid var(--el-border-color-lighter)' : 'none'))
const bodyDividerStyle = computed(() => (props.showBodyVerticalDivider ? '1px solid var(--el-border-color-extra-light)' : 'none'))
...
function resolveEditorType(column: TableColumn) {
  ...
  if (['input', 'select', 'boolean', 'date', 'datetime', 'dropdown'].includes(String(column.type ?? ''))) {
    return column.type as 'input' | 'select' | 'boolean' | 'date' | 'datetime' | 'dropdown'
  }
  return 'none'
}
...
function getDataColumnFromElementColumn(column: { property?: string }) {
  if (!column.property) {
    return undefined
  }

  return activeColumns.value.find((item) => item.key === column.property)
}
...
function handleElementCellClick(
  row: Record<string, unknown>,
  elementColumn: { property?: string },
  _cell: HTMLElement,
  event: MouseEvent
) {
  ...
  const column = getDataColumnFromElementColumn(elementColumn)
  if (column) {
    activateCell(row, column, event)
  }
}

function handleElementCellDblclick(
  row: Record<string, unknown>,
  elementColumn: { property?: string },
  _cell: HTMLElement,
  event: MouseEvent
...
function normalizePastedValue(column: TableColumn, value: string) {
  ...
  if (resolveEditorType(column) === 'select') {
...
function updateShellWidth() {
  shellWidth.value = shellRoot.value?.clientWidth ?? 0
...
function estimateColumnContentWidth(column: TableColumn) {
  ...
  return Math.ceil(Math.max(minWidth, Math.min(maxContentWidth + 48, 640)))
}
...
function resolveColumnMinWidth(column: TableColumn) {
  return resolveColumnWidth(column) ? undefined : column.minWidth ?? 140
}

function handleRowDragStart(row: Record<string, unknown>) {
...
function handleRowDrop(targetRow: Record<string, unknown>) {
  if (draggingRowKey.value === null) {
    return
  }
...
function resolveCellClassName({ row, column }: { row: Record<string, unknown>; column: { property?: string } }) {
  ...
  if (selectedMap.has(cellKey)) {
    ...
    const hasLeft = leftColumn && selectedMap.has(getDirtyKey(rowKey, leftColumn.key))
...
<template>
  <div ref="tableRoot" class="x-table" :class="{ 'is-fill-height': fillHeight }" @click="closeContextMenu">
      ...
      <div class="x-table__toolbar-right">
        ...
        <div class="x-table__tool-groups">
          <div class="x-table__tool-group">
            <ElPopover placement="bottom" :width="500" trigger="click" :popper-style="{ maxHeight: columnPanelMaxHeight, overflow: 'hidden' }">
              <template #reference>
                <span class="x-table__tooltip-anchor" title="列设置">
                  <ElButton class="x-table__icon-button" aria-label="列设置">
                    <i class="ri-list-settings-line" aria-hidden="true" />
                  </ElButton>
                </span>
              </template>

              <div class="x-table__column-panel" :style="{ maxHeight: columnPanelMaxHeight }">
    ...
    <div ref="shellRoot" class="x-table__shell" tabindex="0" @paste="handleTablePaste" @focusout="handleShellFocusOut" @scroll.capture="updateSelectionHandlePosition">
      <ElTable
        v-loading="loading"
        :data="pagedRows"
        :row-key="rowKey"
        ...
        @cell-click="handleElementCellClick"
        @cell-dblclick="handleElementCellDblclick"
      >
        <template #empty>
          <div class="x-table__empty">
            <img class="x-table__empty-image" :src="emptyImage || defaultEmptyImage" alt="" />
            <span class="x-table__empty-text">{{ emptyText }}</span>
          </div>
        </template>

        <ElTableColumn v-if="draggableRows" label="" width="56" fixed="left" align="center">
          <template #default="{ row }">
            <div
              class="x-table__row-dragger"
              draggable="true"
              @dragstart="handleRowDragStart(row)"
              @dragover.prevent="handleRowDragOver(row, $event)"
              @drop.prevent="handleRowDrop(row)"
        ...
        <ElTableColumn v-if="selectable" type="selection" width="52" fixed="left" />
        <ElTableColumn v-if="showIndex" type="index" label="#" width="62" fixed="left" />
        ...
        >
          <template #default="{ row }">
            ...
            >
              <slot :name="`cell-${column.key}`" :row="row" :value="row[column.key]" :editable="isEditableColumn(column)">
                <ElInput
                ...
                <span v-else-if="resolveEditorType(column) === 'select' || resolveEditorType(column) === 'dropdown'">
        ...
        <ElTableColumn v-if="showActions" label="操作" :min-width="actionsMinWidth" fixed="right" align="center">
          <template #default="{ row }">
    ...
    >
      ...
      <ElTable :data="dropdownData" border height="280" @row-dblclick="selectDropdownRow">
        <ElTableColumn
    ...
    >
      ...
      <button type="button" @click="insertRowByActiveCell('above')">向上插入一行</button>
      <button type="button" @click="insertRowByActiveCell('below')">向下插入一行</button>
    </div>
...
.x-table.is-fill-height .x-table__shell :deep(.el-table) {
  flex: 1 1 auto;
  height: 100%;
  min-height: 0;
}

.x-table.is-fill-height .x-table__shell :deep(.el-table__inner-wrapper) {
  flex: 1 1 auto;
  height: 100%;
  min-height: 0;
  overflow: hidden;
...
.x-table.is-fill-height .x-table__shell :deep(.el-table__body-wrapper) {
  flex: 1 1 auto;
  min-height: 0 !important;
  overflow: hidden !important;
}

.x-table.is-fill-height .x-table__shell :deep(.el-scrollbar) {
  height: 100%;
}

.x-table.is-fill-height .x-table__shell :deep(.el-scrollbar__wrap) {
...
.x-table__body :deep(.el-table) {
  --el-table-tr-bg-color: var(--x-table-zebra-stripe-color) !important;
}
...
.x-table__cell.is-dirty::after {
  border-right: 7px solid var(--el-color-warning);
  border-top: 7px solid var(--el-color-warning);
  content: '';
  position: absolute;
  right: -2px;
  top: -2px;
