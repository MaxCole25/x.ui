...
<template>
  <Story title="组件/登录 Login" group="components">
    <Variant title="交互式验收">
    ...
    <Variant title="基础账号登录">
      <div class="login-story__narrow">
        <XLogin title="系统登录" description="只保留用户名、密码和注册事件。" @login="handleLogin" @register="writeLog('基础示例：注册')" />
      </div>
    </Variant>

    <Variant title="完整验证入口">
      <div class="login-story__narrow">
        <XLogin
          title="安全登录"
          description="展示图像验证、字母识别、微信登录和短信验证入口。"
          enable-image-captcha
          enable-letter-captcha
          enable-wechat-login
          enable-sms-login
          letter-captcha-text="M9TX"
          @login="handleLogin"
          @register="writeLog('完整示例：注册')"
        />
      </div>
    </Variant>

    <Variant title="Logo 左右布局">
      <div class="login-story__grid">
        <XLogin title="左侧 Logo" description="名称和介绍区域会自动撑满剩余宽度。" logo-position="left" />
        <XLogin title="右侧 Logo" description="适合品牌图形靠右展示的登录卡片。" logo-position="right" />
      </div>
    </Variant>

    <Variant title="左侧标签和主题色">
      <div class="login-story__narrow">
        <XLogin
          title="主题登录"
          description="标签在左侧，配色由业务应用注入。"
          label-position="left"
          accent-color="#8a4b12"
