...
const colorStyle = computed<CSSProperties>(() => createColorStyle(props.colors))
...
function createColorStyle(colors?: FileDiskColors) {
...
<template>
  <section
    class="x-file-disk"
    :class="{ 'is-dragging': dragging, 'is-busy': isBusy }"
    :style="colorStyle"
...
.x-file-disk {
  ...
  border: 1px solid var(--x-file-disk-border, var(--x-color-border, #cbd5e1));
  border-radius: 8px;
...
.x-file-disk__toolbar {
  align-items: center;
  background: var(--x-file-disk-toolbar-bg, #f8fafc);
  border-bottom: 1px solid var(--x-file-disk-soft-border, #dbe4ee);
  display: flex;
  flex: 0 0 auto;
  gap: 12px;
...
.x-file-disk__tool {
  align-items: center;
  background: var(--x-file-disk-panel-bg, #fff);
  border: 1px solid var(--x-file-disk-border, #cbd5e1);
  border-radius: 6px;
...
.x-file-disk__tool.is-active {
  background: var(--x-file-disk-primary-soft, var(--x-color-primary-soft, #ecfeff));
  border-color: var(--x-file-disk-primary, var(--x-color-primary, #155e75));
  color: var(--x-file-disk-primary, var(--x-color-primary, #155e75));
}
...
.x-file-disk__divider {
  background: var(--x-file-disk-border, #cbd5e1);
  height: 20px;
  width: 1px;
}
...
.x-file-disk__path {
  background: var(--x-file-disk-path-bg, #fff);
  border-bottom: 1px solid var(--x-file-disk-soft-border, #e2e8f0);
  flex: 0 0 auto;
  gap: 2px;
  overflow-x: auto;
  padding: 8px 12px;
...
.x-file-disk__path-back {
  ...
  border: 0;
  border-radius: 4px;
  color: var(--x-file-disk-text, #1f2937);
  cursor: pointer;
  display: inline-flex;
  flex: 0 0 auto;
  height: 26px;
...
.x-file-disk__path-back:hover:not(:disabled) {
  background: var(--x-file-disk-primary-weak, #eef6f8);
  color: var(--x-file-disk-primary, var(--x-color-primary, #155e75));
}
...
.x-file-disk__tile:hover {
  background: var(--x-file-disk-hover-bg, #eff6ff);
  border-color: var(--x-file-disk-selected-border, #93c5fd);
}

.x-file-disk__tile.is-selected {
  background: var(--x-file-disk-selected-bg, #eff6ff);
  border-color: var(--x-file-disk-selected-border, #93c5fd);
}
...
.x-file-disk__thumb {
  ...
  border: 1px solid var(--x-file-disk-soft-border, #dbe4ee);
  border-radius: 5px;
  box-sizing: border-box;
  display: block;
  height: 48px;
...
.x-file-disk__name-input {
  background: var(--x-file-disk-panel-bg, #fff);
  border: 1px solid var(--x-file-disk-primary, var(--x-color-primary, #155e75));
  border-radius: 4px;
...
.x-file-disk__drop-mask {
  align-items: center;
  background: var(--x-file-disk-drop-bg, rgba(14, 116, 144, 0.12));
  border: 2px dashed var(--x-file-disk-primary, var(--x-color-primary, #155e75));
  color: var(--x-file-disk-primary, var(--x-color-primary, #155e75));
  display: flex;
  flex-direction: column;
  font-size: 14px;
...
.x-file-disk__context-divider {
  border-top: 1px solid var(--x-file-disk-soft-border, #e2e8f0);
  margin: 5px 0;
}

.x-file-disk__upload-panel {
  background: var(--x-file-disk-panel-bg, rgba(255, 255, 255, 0.96));
  border: 1px solid var(--x-file-disk-border, #cbd5e1);
  border-radius: 8px;
  bottom: 12px;
...
.x-file-disk__upload-track {
  background: var(--x-file-disk-soft-border, #e2e8f0);
  border-radius: 999px;
  height: 6px;
  overflow: hidden;
}
