...
本规范用于约束 x.ui 后续新增或修改组件时的公开接口命名，包括 Props、事件、插槽、公开类型和 `expose` 方法。x.ui 尚未进入生产依赖，当前按 pre-1.0 策略治理公开接口：旧命名 Props 不保留兼容别名，不添加 `@deprecated` fallback；新增或修改公开接口时直接按本规范命名。
